...
export class WebGLResizePacked extends Upsample implements WebGLOperator {
  ...
  createProgramInfo(handler: WebGLInferenceHandler, inputs: Tensor[]): ProgramInfo {
    // TODO: should handle input length check here.
    if (inputs.length !== 4) {
  ...
  prepare(inputs: Tensor[]): [number[], number[], ReadonlyArray<number>] {
    const x = inputs[0];
    const xDims = x.dims;
...
function createResizeProgramInfo(
  ...
  if (mode === 'linear') {
    ...
    const getSourceFracIndex = `
    vec3 getSourceFracIndex(ivec3 coords, float scale){
      return (vec3(coords) + 0.5)/ vec3(scale) - 0.5;
    }
    ...
    shader = `
        ...
        const vec2 scaleWH = vec2(3.0, 3.0);
        ...
        void main() {
          ...
          // vec3 fracRCX = clamp(sourceFracX, vec3(0.0), vec3(1.0));
          // vec3 fracRCY = clamp(sourceFracY, vec3(0.0), vec3(1.0));
          ...
          // vec4 fracRCY = vec4(sourceFracR.y - float(rr.y),
          //                     sourceFracG.y - float(gg.y),
          //                     sourceFracB.y - float(bb.y),
          //                     sourceFracA.y - float(aa.y));

          // vec4 fracRCX = vec4(sourceFracR.x - float(rr.x),
          //                   sourceFracG.x - float(gg.x),
          //                   sourceFracB.x - float(bb.x),
          //                   sourceFracA.x - float(aa.x));
          ...
          // // // // |
          // // // // w direction
          // // // ivec3 outputPackedX = rc.www + ivec3(0, 1, 2);
          // // // ivec3 outputPackedY = rc.zzz + ivec3(0, 1, 2);

          // // // // for 4x4 output, the value is -0.25, 0.25, 0.75 and 1.25
          // // // vec3 sourceFracX = getSourceFracIndex(outputPackedX, ${scalesHeight}.0);
          // // // vec3 sourceFracY = getSourceFracIndex(outputPackedY, ${scalesWidth}.0);

          // // // ivec3 sourceFloorRCX = ivec3(max(sourceFracX, vec3(0.0)));
          // // // ivec3 sourceCeilRCX = ivec3(
          // // //   min(vec3(${scalesHeight}.0 - 1.0), ceil(sourceFracX)));

          // // // ivec3 sourceFloorRCY = ivec3(max(sourceFracY, vec3(0.0)));
          // // // ivec3 sourceCeilRCY = ivec3(
          // // //   min(vec3(${scalesWidth}.0 - 1.0), ceil(sourceFracY)));

          // // // bool hasNextRow = rc.w < ${outputHeight - 1};
          // // // bool hasNextCol = rc.z < ${outputWidth - 1};

          // // // vec4 topLeft = vec4(
          // // //   getAValue(b, d, sourceFloorRCX.x, sourceFloorRCY.x),
          // // //   hasNextCol ? getAValue(b, d, sourceFloorRCX.x, sourceFloorRCY.y)
          // // //             : 0.0,
          // // //   hasNextRow ? getAValue(b, d, sourceFloorRCX.y, sourceFloorRCY.x)
          // // //             : 0.0,
          // // //   (hasNextRow && hasNextCol) ?
          // // //     getAValue(b, d, sourceFloorRCX.y, sourceFloorRCY.y) : 0.0);

          // // // vec4 bottomLeft = vec4(
          // // //   getAValue(b, d, sourceCeilRCX.x, sourceFloorRCY.x),
          // // //   hasNextCol ? getAValue(b, d, sourceCeilRCX.x, sourceFloorRCY.y)
          // // //             : 0.0,
          // // //   hasNextRow ? getAValue(b, d, sourceCeilRCX.y, sourceFloorRCY.x)
          // // //             : 0.0,
          // // //   (hasNextRow && hasNextCol) ?
          // // //     getAValue(b, d + 1, sourceCeilRCX.y, sourceFloorRCY.y) : 0.0);

          // // // vec4 topRight = vec4(
          // // //   getAValue(b, d, sourceFloorRCX.x, sourceCeilRCY.x),
          // // //   hasNextCol ? getAValue(b, d, sourceFloorRCX.x, sourceCeilRCY.y)
          // // //             : 0.0,
          // // //   hasNextRow ? getAValue(b, d, sourceFloorRCX.y, sourceCeilRCY.x)
          // // //             : 0.0,
          // // //   (hasNextRow && hasNextCol) ?
          // // //     getAValue(b, d, sourceFloorRCX.y, sourceCeilRCY.y) : 0.0);

          // // // vec4 bottomRight = vec4(
          // // //   getAValue(b, d, sourceCeilRCX.x, sourceCeilRCY.x),
          // // //   hasNextCol ? getAValue(b, d, sourceCeilRCX.x, sourceCeilRCY.y)
          // // //             : 0.0,
          // // //   hasNextRow ? getAValue(b, d, sourceCeilRCX.y, sourceCeilRCY.x)
          // // //             : 0.0,
          // // //   (hasNextRow && hasNextCol) ?
          // // //     getAValue(b, d, sourceCeilRCX.y, sourceCeilRCY.y) : 0.0);
          ...
          // // // vec4 top = mix(topLeft, topRight, fracRCY.xyxy);
          // // // vec4 bottom = mix(bottomLeft, bottomRight, fracRCY.xyxy);
          // // // vec4 newValue = mix(top, bottom, fracRCX.xxyy);

          // // // //outputColor = vec4(float(hasNextCol), float(hasNextRow), 0, 0);
          // // // outputColor = vec4(topRight);
          // // // //outputColor = vec4(sourceFloorRCY, 0);
        }
      `;

  } else {
    // TODO: finish nearest mode and cubic mode.
  }
  return {
    inputLayouts: [inputLayout],
    outputLayout,
...
//     getOriginalCoordinate: Upsample.GetOriginalCoordinateFunc, mappingOrigin: number[], mappingExtrapolation:
