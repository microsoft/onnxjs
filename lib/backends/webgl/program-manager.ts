...
export class ProgramManager {
  ...
  build(programInfo: ProgramInfo): Artifact {
    return this.profiler.event('backend', 'ProgramManager.build', () => {
      ...
      //         vec2 uv = (vec2(row, col) + halfCR) / vec2(2.0, 4.0);
      //         return sampleTexture(A, uv);
      //       }

      //       float getA(int batch, int row, int col) {
      //         return getA(col, batch);
      //       }
      ...
      //         outputColor = vec4(getA(rc.x,r, c),
      //       rEdge ? 0. : getA(rc.x,rp1, c),
      //       cEdge ? 0. : getA(rc.x,r, cp1),
