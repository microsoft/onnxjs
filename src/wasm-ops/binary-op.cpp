...
float xor_core(const float &a, const float &b) {
  return static_cast<float>(static_cast<bool>(a) ^ static_cast<bool>(b));
}
float or_core(const float &a, const float &b) {
  return static_cast<float>(static_cast<bool>(a) || static_cast<bool>(b));
}
float and_core(const float &a, const float &b) {
  return static_cast<float>(static_cast<bool>(a) && static_cast<bool>(b));
}
float prelu_core(const float &a, const float &b) { return a >= 0 ? a : a * b; }

// Core binary operator implementation
void binary_f32_imp(const float *input1, const int32_t &length1,
                    const int32_t &rank1, const std::vector<int32_t> &dims1,
                    const float *input2, const int32_t &length2,
                    const int32_t &rank2, const std::vector<int32_t> &dims2,
...
void add_f32(void *data) { binary_f32_imp_wrapper(data, add_core); }
void sub_f32(void *data) { binary_f32_imp_wrapper(data, sub_core); }
void mul_f32(void *data) { binary_f32_imp_wrapper(data, mul_core); }
void div_f32(void *data) { binary_f32_imp_wrapper(data, div_core); }
void xor_f32(void *data) { binary_f32_imp_wrapper(data, xor_core); }
void or_f32(void *data) { binary_f32_imp_wrapper(data, or_core); }
void and_f32(void *data) { binary_f32_imp_wrapper(data, and_core); }
void prelu_f32(void *data) { binary_f32_imp_wrapper(data, prelu_core); }