...
void matmul_f32(void *data) {
  ...
  matmul_f32_imp(input_1, dims_1, rank_1, input_2, dims_2, rank_2, output, output_length, output_dims, output_rank);
}

// Core operator implementation
void matmul_f32_imp(const float *input_1, const int32_t *dims_1, const int32_t rank_1, const float *input_2, const int32_t *dims_2, const int32_t rank_2, float *output, const int32_t output_length, const int32_t *output_dims, const int32_t output_rank) {
  ...
  // multi-D matrices
	...
	const std::vector<int32_t> strides_1 = ShapeUtils::compute_strides(dims_1_vector);  
	
	std::vector<int32_t> dims_2_vector(rank_2);
	for (int32_t r = 0; r < rank_2; ++r) {
		dims_2_vector[r] = dims_2[r];
	}
	const std::vector<int32_t> strides_2 = ShapeUtils::compute_strides(dims_2_vector);

	std::vector<int32_t> broadcasted_indices = std::vector<int32_t>(output_rank);
	...
	for (int32_t i = 0; i < num_matrices; i++) {
	    ...
	    // This matrix is not 2D, so no need to find appropriate start_offset		
			...
			original_indices_1 = BroadcastUtils::broadcasted_to_original_indices(broadcasted_indices, dims_1_vector);
			original_offset_1 = ShapeUtils::indices_to_offset(strides_1, original_indices_1);
		...
		// This matrix is not 2D, so no need to find appropriate start_offset		
			...
			original_indices_2 = BroadcastUtils::broadcasted_to_original_indices(broadcasted_indices, dims_2_vector);
			original_offset_2 = ShapeUtils::indices_to_offset(strides_2, original_indices_2);
			input_2_traverse = input_2 + original_offset_2;
		}
		
		// process this 2D component alone		
		matmul2D_f32(input_1_traverse, input_2_traverse, output + (i*M*N), M, K, N);	
	}
  }
}

// Core functionality implementation
void matmul2D_f32(const float *input_1, const float *input_2, float *output, const int32_t M, const int32_t K, const int32_t N) {
